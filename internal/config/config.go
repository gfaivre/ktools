...
func Load() (*Config, error) {
	...
	// Config paths: ~/.config/ktools/ ou ~/.ktools/
	home, err := os.UserHomeDir()
	if err != nil {
		return nil, fmt.Errorf("impossible de trouver le home directory: %w", err)
	}
...
func (c *Config) Validate() error {
	if c.APIToken == "" {
		return fmt.Errorf("api_token requis (config ou KTOOLS_API_TOKEN)")
	}
	if c.DriveID == 0 {
		return fmt.Errorf("drive_id requis (config ou KTOOLS_DRIVE_ID)")
	}
	return nil
}
